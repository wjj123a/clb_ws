...
cd "$WS_DIR"
"$WS_DIR/stop_sim.sh" --quiet || true
echo "Starting fusion mapping scene."
echo "Save the current map with: rosrun map_server map_saver -f <map_name> map:=/map"
echo "RTAB-Map database path defaults to: $HOME/.ros/rtabmap_r1.db"
exec roslaunch r1 sim_mapping_fusion.launch "$@"
