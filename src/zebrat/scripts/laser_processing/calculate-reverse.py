...
def calculate(n_scans, resolution, length = 1, width = 0.6, lidar_pos = (0.3, 0.9)):
    ...
    for i in range(int(w/2/res)):
        angle = math.pi + math.atan((i)* res/y)
    ...
    for i in range(int(w/2/res)):
        j = int(w/2/res)-i
        angle = 2* math.pi - math.atan(j*res/(l-y))
        index.append(round(angle/inc))
