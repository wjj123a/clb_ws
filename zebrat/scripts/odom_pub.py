...
pub_name    = "odom_topic"
imu_name    = "imu_data"
joint_states_name   = "/zebrat/joint_states"
# 定义起始坐标
start_x     = 0.0
start_y     = 0.0
start_z     = 0
class odom_class:
    ...
    def callback_join(self, data):
        ...
        #try:
        # 编码器 测速
        lrw_topic = data.name.index('rl_axle')
        rrw_topic = data.name.index('rr_axle')
        # 平均速度
        velocity = (data.velocity[lrw_topic] + data.velocity[rrw_topic])/(2 * 5.97015)
        # 平均路程
